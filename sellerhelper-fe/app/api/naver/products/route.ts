...
function extractProductIdsFromOrders(data: unknown): string[] {
  ...
  const visit = (obj: unknown) => {
    ...
    const o = obj as { productId?: unknown; productid?: unknown; productOrder?: unknown; productOrders?: unknown[]; order?: unknown };
    const pid = o.productId ?? o.productid;
    if (pid && String(pid).trim()) ids.add(String(pid).trim());
    if (o.productOrder) visit(o.productOrder);
    if (Array.isArray(o.productOrders)) o.productOrders.forEach(visit);
    else if (o.order) visit(o.order);
  };
