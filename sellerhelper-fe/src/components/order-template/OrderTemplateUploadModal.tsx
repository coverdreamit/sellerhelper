...
interface OrderTemplateUploadModalProps {
  onClose?: () => void;
  onSave?: (supplierId: string, columnKeys: string[]) => void;
  supplierId?: string;
}

export default function OrderTemplateUploadModal({ onClose, onSave, supplierId: initialSupplierId = '1' }: OrderTemplateUploadModalProps) {
  const [step, setStep] = useState(1);
  const [supplierId, setSupplierId] = useState(initialSupplierId);
  const [excelColumns, setExcelColumns] = useState<string[]>([]);
  const [mapping, setMapping] = useState<Record<string, MappingItem>>({});
  ...
  const handleChangeMapping = (excelCol: string, value: string) => {
    setMapping((prev) => ({
      ...prev,
  ...
  const hasUnmapped = Object.values(mapping).some((m: MappingItem) => !m.key);

  const handleSave = () => {
    const columnKeys = Object.values(mapping)
      .map((m: MappingItem) => m.key)
      .filter(Boolean);
