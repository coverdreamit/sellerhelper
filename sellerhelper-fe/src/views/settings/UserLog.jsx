...
const mockImportHistory = [
  { id: 1, date: '2025-02-19 15:00', target: '상품 목록', store: '스마트스토어', status: '성공', count: 12 },
  { id: 2, date: '2025-02-19 09:00', target: '주문 목록', store: '스마트스토어', status: '성공', count: 8 },
  { id: 3, date: '2025-02-18 18:00', target: '상품 목록', store: '쿠팡', status: '성공', count: 24 },
];
              ...
              <tbody>
                {mockOrderHistory.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.items}건</td>
                    <td>
                      <span className={`badge badge-${row.status === '완료' ? 'active' : 'inactive'}`}>
                        {row.status}
                      </span>
                    </td>
                  </tr>
              ...
              <tbody>
                {mockImportHistory.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.count}건</td>
                    <td>
                      <span className={`badge badge-${row.status === '성공' ? 'active' : 'inactive'}`}>
                        {row.status}
                      </span>
                    </td>
                  </tr>
