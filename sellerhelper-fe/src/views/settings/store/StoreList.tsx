...
function SortableStoreRow({ store, onDisconnect, onAuth, onSetting, onToggleEnable }) {
  ...
  return (
    <tr ref={setNodeRef} style={style} className={isDragging ? 'store-row-dragging' : ''}>
      ...
      <td>
        {userStore?.lastSyncAt ? new Date(userStore.lastSyncAt).toLocaleString('ko-KR') : '-'}
      </td>
      <td>
...
export default function StoreList() {
  ...
  return (
    <div className="settings-page">
      ...
      <StoreAuthModal storeCode={authStoreCode} onClose={() => setAuthStoreCode(null)} />

      <StoreSettingModal storeCode={settingStoreCode} onClose={() => setSettingStoreCode(null)} />

      {showAddModal && <StoreAddModal onAdd={handleAdd} onClose={() => setShowAddModal(false)} />}
    </div>
  );
}
