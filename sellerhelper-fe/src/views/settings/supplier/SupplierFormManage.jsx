...
export default function SupplierFormManage({ vendorId: initialVendorId }) {
  ...
  const handleUploadSave = useCallback((supplierId, columnKeys) => {
    ...
    setFormCards((prev) => {
      ...
      return [...prev, { id: Date.now(), name, supplierId, supplier, columns: columnKeys, useYn: 'Y', updated }];
    });
    setShowUpload(false);
  }, []);
        ...
        <div className="settings-toolbar">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button
              type="button"
              className="btn"
              onClick={() => setShowAutoMappingSetting(true)}
            >
              자동매핑 설정
            </button>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setPreviewModalForm({
                          ...
                          columnKeys: f.columns?.length ? f.columns : loadFormFromStorage(f.supplierId) || DEFAULT_COLUMN_KEYS,
                        })
                      }
                    >
                      미리보기
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEditModalForm({
                          ...
                          supplierName: f.supplier,
                          initialColumnKeys: f.columns?.length ? f.columns : loadFormFromStorage(f.supplierId) || DEFAULT_COLUMN_KEYS,
                        })
                      }
                    >
                      편집
