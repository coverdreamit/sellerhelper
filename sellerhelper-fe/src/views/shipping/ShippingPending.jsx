...
const mockPending = [
  { orderId: 'ORD-2024-001', store: '스마트스토어', buyer: '홍*동', amount: 45000, orderDate: '2024-02-06 14:32' },
  { orderId: 'ORD-2024-005', store: 'G마켓', buyer: '김*수', amount: 125000, orderDate: '2024-02-06 10:05' },
  { orderId: 'ORD-2024-007', store: '쿠팡', buyer: '이*준', amount: 32000, orderDate: '2024-02-06 11:20' },
];
...
export default function ShippingPending() {
  ...
  const filtered =
    storeTab && storeTabs.length > 0
      ? mockPending.filter((s) => (s.store ?? '스마트스토어') === storeTab)
      : mockPending;
      ...
      <section className="settings-section">
        ...
        <div className="settings-toolbar">
          ...
          <div className="product-list-actions">
            <Link to="/shipping/invoice" className="btn btn-primary">송장 일괄 입력</Link>
          </div>
        </div>
        <div className="settings-table-wrap">
          <table className="settings-table">
            <thead>
              <tr>
                <th><input type="checkbox" title="전체선택" /></th>
                <th>주문번호</th>
                <th>스토어</th>
                <th>수령인</th>
                <th>주문금액</th>
            ...
            <tbody>
              {paged.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: 24, textAlign: 'center' }}>출고 대기 건이 없습니다.</td></tr>
              ) : (
                paged.map((s) => (
                  <tr key={s.orderId}>
