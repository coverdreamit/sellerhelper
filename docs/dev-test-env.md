...
**연결 확인**: FE는 `NEXT_PUBLIC_API_URL`(기본 `http://localhost:5001`)로 BE를 직접 호출합니다.  
BE를 **5001 포트**로 띄워야 합니다. (기본값이 5001이므로 별도 설정 없이 실행하면 됨.  
이전에 5080으로 띄웠다면 백엔드 재시작 후 브라우저에서 주문 목록 등 호출 시 BE 로그에 요청이 찍히는지 확인하세요.)
